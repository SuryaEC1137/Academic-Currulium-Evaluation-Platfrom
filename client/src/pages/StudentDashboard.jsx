...
const Counter = memo(({ value }) => {
    ...
    useEffect(() => {
        ...
        let incrementTime = Math.max(totalMiliseconds / Math.abs(end - start), 16);
        let timer = setInterval(() => {
            setCount(prev => {
                const next = prev + (end > prev ? 1 : -1);
...
const StudentDashboard = () => {
    ...
    }, [debouncedSearch, activeDept, courses, activeTab]);
        ...
        <RouteTransition>
            <div className="min-h-screen pt-28 pb-12 px-4 bg-slate-50 dark:bg-[#030712] transition-colors duration-700 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                            <AnimatePresence mode='popLayout'>
                                {filteredCourses.slice(0, displayLimit).map((course, idx) => (
                                    <CourseCard
                                        key={course._id}
                                        course={course}
                                        idx={idx}
                    ...
                    {!loading && filteredCourses.length > displayLimit && (
                        <div className="mt-20 flex justify-center">
                            <motion.button
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                                onClick={() => setDisplayLimit(prev => prev + 12)}
                                className="px-12 py-5 rounded-2xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-indigo-600 dark:text-indigo-400 font-black text-xs uppercase tracking-[0.3em] shadow-xl hover:shadow-indigo-500/10 transition-all flex items-center gap-4 group"
                            >
                                <span>Load Strategic Catalog</span>
