...
const Navbar = () => {
    ...
    return (
        <motion.nav
            initial={{ y: -100 }}
            animate={{ y: 0 }}
            className={`fixed top-0 w-full z-50 transition-[background-color,border-color,box-shadow,backdrop-filter] duration-500 theme-transition ${scrolled || !isAuthPage ? 'bg-white/80 dark:bg-[#030712]/80 backdrop-blur-2xl shadow-lg shadow-indigo-500/5 border-b border-slate-200/50 dark:border-white/5' : 'bg-transparent border-transparent'}`}
        >
                        ...
                        {userInfo ? (
                            <div className="flex items-center gap-4">
                                ...
                                */}
                                ...
                                <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 relative overflow-hidden">
                                    <Link
                                        to={userInfo.role === 'admin' ? '/admin' : (userInfo.role === 'faculty' ? '/faculty/dashboard' : '/dashboard')}
                                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all duration-500 relative z-10 group/link ${location.pathname.includes('dashboard') || location.pathname === '/admin' ? 'text-white' : 'text-slate-500 dark:text-gray-400 hover:text-indigo-500'}`}
                                    >
                                        ...
                                        <ChartBarIcon className="w-4 h-4 relative z-10 group-hover/link:scale-110 transition-transform" />
                                        <span className="relative z-10">{userInfo.role === 'admin' ? 'Administration' : (userInfo.role === 'faculty' ? 'Faculty Center' : 'Dashboard')}</span>
                                    </Link>

                                    {userInfo.role === 'faculty' && (
                                        <Link
                                            to="/dashboard"
                                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-all duration-500 relative z-10 group/link ${location.pathname === '/dashboard' ? 'text-white' : 'text-slate-500 dark:text-gray-400 hover:text-indigo-500'}`}
                                        >
